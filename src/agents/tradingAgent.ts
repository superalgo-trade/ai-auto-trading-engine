...
5. **灵活的仓位管理**：您的仓位大小基于风险和信号强度。**单笔交易风险建议在账户净值的8-15%之间**，根据信号强度选择：普通信号使用8-10%，强信号使用10-15%。最多同时持有${RISK_PARAMS.MAX_POSITIONS}个持仓。
...
- **杠杆控制（严格限制）**：最多使用5-15倍杠杆。
  ...
  * **禁止**使用超过15倍杠杆，无论信号多强
- **仓位大小（积极进取）**：
  * 单笔交易风险建议在账户净值的8-15%之间，根据信号强度选择：
    - 普通信号（2个时间框架一致）：使用8-10%仓位
    - 良好信号（3个时间框架一致）：使用10-12%仓位
    - 强信号（4个或更多时间框架一致）：使用12-15%仓位
...
4. **评估新交易机会（在满足基本条件时积极入场）**：
   ...
   - **仓位大小原则**：根据信号强度使用8-15%的仓位，不要过于保守
   - **做多和做空机会的识别**：
...
5. **仓位大小和杠杆计算**：
   - 单笔交易仓位 = 账户净值 × 8-15%（根据信号强度）
     * 普通信号（2个时间框架一致）：8-10%
     * 良好信号（3个时间框架一致）：10-12%
     * 强信号（4个或更多时间框架一致）：12-15%
   - 杠杆选择（根据信号强度灵活选择）：
     * 5-8倍：2个时间框架一致的普通信号
     * 8-12倍：3个时间框架一致的良好信号
     * 12-15倍：4个或更多时间框架强烈一致的优质信号
   - **举例**：如果账户净值1000 USDT，普通信号使用10%仓位=100 USDT，杠杆8倍
